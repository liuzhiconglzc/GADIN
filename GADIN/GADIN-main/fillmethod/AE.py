...
def ae_impute (data_x, gain_parameters):
  ...
  #
  # D_W2 = tf.Variable(xavier_init([h_dim, h_dim]))# TODO hmf 适当减少隐藏层节点数：注意前者是输入个数，后者是输出个数
  ...
  G_W2 = tf.Variable(xavier_init([h_dim, h_dim-2]))
...
def ae_impute_fill(data_x, gain_parameters):
  ...
  for it in tqdm(range(iterations)):
    ...
    _, G_loss_curr = \
      sess.run([G_solver, MSE_loss],
               feed_dict={X: X_mb, M: M_mb})
    # TODO 2 *********************************************************  end
    rmse_list.append(round(G_loss_curr, 4))
